...
function getTests(win){
  return [
    {
      name: "App version is v1.0.26",
      run(){ assertEqual(win.APP_VERSION, "1.0.26"); }
    },
    {
      name: "data module bridge exposes Raven constants",
      run(){
        assertEqual(win.STORAGE_KEY, "lastAsylumPlagueToolsRavenGearV1");
    ...
    {
      name: "core folder modules expose data and action registry exports",
      async run(){
        const cacheBust = Date.now();
        const dataModule = await import(`../src/app/core/data.js?test=${cacheBust}`);
        const actionModule = await import(`../src/app/core/action-registry.js?test=${cacheBust}`);
        assertEqual(dataModule.RAVEN_ITEMS.length, 6);
        assertEqual(dataModule.DEFAULT_CHEST_LEVELS, [5,4,3,2,1]);
    ...
    },
    {
      ...
      run(){
        ...
        assertEqual(payload.appVersion, "1.0.26");
    ...
    },
    {
      ...
      run(){
        ...
        assertEqual(backup.state.settings, {hideMoreInformationSections:true,hideAdvancedSections:true});
