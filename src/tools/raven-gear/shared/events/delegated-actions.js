...
export function createDelegatedActionRuntimeModule(context = {}){
  const delegatedActionNames = [
    ...
    "setInventoryAdvancedMode","setInventory","adjustInventory","setInventoryLevelRange","toggleInventoryLevelRange","setInventoryHighLevel","setInventoryTechPoints","openTechPointsModal","setHideUnacquiredInventory","setInventoryEquippedOnly","setUpgradedOwnedViewMode",
  ...
  function registerDelegatedActions(){
    ...
    registry.registerActions({
      ...
      refreshChestInputs(){
        ...
        if(typeof renderRandomInputs==="function") renderRandomInputs();
        if(typeof renderGuaranteedInputs==="function") renderGuaranteedInputs();
        if(typeof renderAll==="function") renderAll();
      },
      toggleAuditTotalView({event,element}){
        const line=element ? element.closest('[data-audit-total-line]') : null;
        if(!line) return;
        const raw=line.querySelector('[data-audit-total-raw]');
        const simplified=line.querySelector('[data-audit-total-simplified]');
  ...
  function bindDelegatedUiEvents(options){
    if(window.__lapDelegatedUiEventsBound) return;
    window.__lapDelegatedUiEventsBound=true;

    // When a tool passes an AbortController signal, the listeners are added with
    // it so the tool can remove them on unmount; on abort we clear the flag so a
    // later mount rebinds. Without a signal (hub shell) the listeners are permanent.
    ...
    if(signal) signal.addEventListener("abort",()=>{ window.__lapDelegatedUiEventsBound=false; },{once:true});
  }
