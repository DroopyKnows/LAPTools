...
export function createMathLevels(runtime, api){
  ...
  function equivalentFromHigherAndLower(obj,level){
    ...
    allItemLevels.forEach(sourceLevel=>{
      const qty=api.getQty(obj,sourceLevel);
      if(!qty) return;
  ...
  function excelDisplayWhole(value){
    const rounded=api.excelWholeCount(value);
  ...
  function displayWholeByLevels(source,levels){
    return levels.map(level=>api.excelDisplayWhole(source[level]));
  }

  function mergedOwnedWithAddtl(owned,addtl){
    const merged={};
    allItemLevels.forEach(level=>{
      const total=api.getQty(owned,level)+api.getQty(addtl,level);
  ...
  function fractionalRemainderFromSimplified(simplified){
    const remainder={};
    Object.keys(simplified || {}).forEach(level=>{
      const value=api.safeNum(simplified[level]);
      const whole=api.excelRoundWhole(value);
  ...
  function addDisplayRows(rows){
    const total={};
    rows.forEach(row=>api.addObjects(total,row));
    return total;
  }
