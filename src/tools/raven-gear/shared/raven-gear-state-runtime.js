// Raven Gear state runtime helpers

export function createRavenGearStateRuntimeModule({
  allItemLevels,
  chestLevels,
  ravenItems,
  ...
  function normalizeWhatIfScenario(source){
    ...
    return {
      item,
      targetLevel:toSafeInteger(src.targetLevel,7,1,maxItemLevel),
  ...
  function normalizeInventoryLevelRangeMap(source){
  const out={};
  const src=isPlainObject(source) ? source : {};
  ravenItems.forEach(item=>{
    out[item.name]=src[item.name]==="8-12" ? "8-12" : "1-7";
  });
    return out;
  }

  function normalizeInventoryHighLevelMap(source){
    const out={};
    const src=isPlainObject(source) ? source : {};
    ravenItems.forEach(item=>{
      const value=toSafeInteger(src[item.name],0,0,12);
      if(value>=8 && value<=12) out[item.name]=value;
    });
    return out;
  }

  function normalizeInventoryTechPointsMap(source){
    const out={};
    const src=isPlainObject(source) ? source : {};
    ravenItems.forEach(item=>{
      const value=toSafeInteger(src[item.name],0,0);
      if(value>0) out[item.name]=value;
    });
    return out;
  }
  ...
  function normalizeInventoryState(source){
    ...
    const active=normalizeItemLevelMap(src.active ?? legacy.inventory, allItemLevels);
    const highLevelByItem=normalizeInventoryHighLevelMap(src.highLevelByItem ?? legacy.inventoryHighLevelByItem);
    ravenItems.forEach(item=>{
      if(highLevelByItem[item.name]) return;
      const owned=active[item.name] || {};
      const detected=[12,11,10,9,8].find(level=>Number(owned[level]||0)>0);
      if(detected) highLevelByItem[item.name]=detected;
    });
    return {
      active,
      ...
      levelRangeByItem:normalizeInventoryLevelRangeMap(src.levelRangeByItem ?? legacy.inventoryLevelRangeByItem),
      highLevelByItem,
      techPointsByItem:normalizeInventoryTechPointsMap(src.techPointsByItem ?? legacy.inventoryTechPointsByItem)
    };
  }

  // Snapshot visibility setting: on (both states) | off | onlyClosed | onlyOpen.
  ...
  function createEmptyScannerUiState(){
    return {
      lastScan:null,
      status:"",
      statusType:"",
  ...
  function normalizeUiState(source){
    ...
    ui.bagScanner=createEmptyScannerUiState();
    return ui;
  }
  ...
  function attachStateAliases(target){
    ...
    defineAlias(target,"inventoryLevelRangeByItem",()=>target.inventory.levelRangeByItem,value=>target.inventory.levelRangeByItem=normalizeInventoryLevelRangeMap(value));
    defineAlias(target,"inventoryHighLevelByItem",()=>target.inventory.highLevelByItem,value=>target.inventory.highLevelByItem=normalizeInventoryHighLevelMap(value));
    defineAlias(target,"inventoryTechPointsByItem",()=>target.inventory.techPointsByItem,value=>target.inventory.techPointsByItem=normalizeInventoryTechPointsMap(value));
