...
function preserveImportedUiStructures(normalizedState, importedState){
  if(!domainIsPlainObject(normalizedState)) return normalizedState;
  if(domainIsPlainObject(importedState && importedState.ui)){
    normalizedState.ui=JSON.parse(JSON.stringify(importedState.ui));
  }
  return normalizedState;
}

function stripCompatibilityOnlySettingsDefaults(normalizedState, importedState){
  const sourceSettings=domainIsPlainObject(importedState && importedState.settings) ? importedState.settings : importedState;
  const normalizedSettings=domainIsPlainObject(normalizedState && normalizedState.settings) ? normalizedState.settings : null;
  if(!normalizedSettings) return normalizedState;

  const sourceHadDismissedMessages=domainIsPlainObject(sourceSettings) && Object.prototype.hasOwnProperty.call(sourceSettings,"dismissedMessages");
  const dismissedMessages=normalizedSettings.dismissedMessages;
  if(!sourceHadDismissedMessages && domainIsPlainObject(dismissedMessages) && Object.keys(dismissedMessages).length===0){
    delete normalizedSettings.dismissedMessages;
  }
  return normalizedState;
}
...
function normalizeBackupPayloadDomain(payload,{
  ...
  try{
    ...
    const normalizedState=preserveImportedUiStructures(
      stripCompatibilityOnlySettingsDefaults(normalizeState(migrated),importedState),
      importedState
    );
