...
export function createAppNavigationRuntimeModule(context = {}){
  ...
  function openGlobalMenu(){
    ...
    // Remember the trigger (usually the hamburger) so focus can return there on close.
    menuOpener=(document.activeElement && typeof document.activeElement.focus==="function") ? document.activeElement : null;
  ...
  function closeGlobalMenu(){
    const panel=document.getElementById("globalMenuPanel");
    const scrim=document.getElementById("menuScrim");
    // If focus is inside the panel, return it to the trigger BEFORE hiding, so focus is
    // never trapped on an aria-hidden element (assistive-tech correctness).
    if(panel && panel.contains(document.activeElement)){
      const opener=menuOpener || document.querySelector('[data-action="openGlobalMenu"]');
  ...
  function applyGlobalDisplaySettings(){
    ...
    // iOS Safari ignores maximum-scale for manual pinch, so also block the pinch
    // gesture events while the guard is active. Block zoom-IN only (e.scale > 1) and
    // always allow zoom-OUT, so a user who somehow ended up zoomed can pinch back to
    // fit without turning the setting off. Bound once; reads a live flag.
    window.__lapZoomGuardActive=zoomGuardOn;
    if(!window.__lapZoomGuardBound){
      window.__lapZoomGuardBound=true;
      const blockZoomIn=e=>{ if(window.__lapZoomGuardActive && e.scale>1) e.preventDefault(); };
      document.addEventListener("gesturestart",blockZoomIn,{passive:false});
      document.addEventListener("gesturechange",blockZoomIn,{passive:false});
    }
  }
